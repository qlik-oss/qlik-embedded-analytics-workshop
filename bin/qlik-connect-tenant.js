...
const main = async () => {
  ...
  const confirmParticipation = await confirm({
    message:
      "Use of this workshop requires your name and email so that the tenant can send you an email invite to join it. Continue?",
    default: true,
  });
  ...
  if (isUser) {
    ...
    if (userCreated) {
      ...
      throw new Error(
        `Workshop setup failed on user creation and invite on tenant.`
      );
    }
  ...
  spinner.succeed(
    "You can now start the workshop by entering npm run start at the prompt"
  );
  process.exit();
};
...
async function assignTenant(email) {
  try {
    const formData = new URLSearchParams();
    formData.append("your_email_address", email);

    const response = await fetch(workshopSettings.tenantAssignmentAutomation, {
      method: "POST",
      headers: {
        "X-Execution-Token": workshopSettings.tenantAssignmentAutomationKey,
    ...
    // Handle the escaped JSON format and extract the tenant data
    if (Array.isArray(jsonResponse) && jsonResponse.length > 0) {
      const parsedData = JSON.parse(jsonResponse[0]);
      return parsedData.tenant;
    }
    return null;
  } catch (error) {
...
async function createTenantOAuthClient(
  tenantHostname,
  codespaceHostname,
  accessToken
) {
  ...
  try {
    ...
    throw new Error(
      `Failed to create OAuth client on ${tenantHostname}`,
      error
    );
  }
}
...
async function createUser(tenantHostname, accessToken, userInput) {
  ...
  try {
    const response = await fetch(
      `${tenantHostname}api/v1/users/actions/invite`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
async function makeUserAdmin(tenantHostname, accessToken, userId) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Response status on set role assignment on user: ${setUserAdmin.status}`
      );
      return false;
    }
    return true;
